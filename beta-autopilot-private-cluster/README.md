...
| binary\_authorization\_evaluation\_mode | Mode of operation for Binary Authorization policy evaluation. Valid values are DISABLED and PROJECT\_SINGLETON\_POLICY\_ENFORCE. | `string` | `""` | no |
...
| gke\_backup | Enable GKE backup | `bool` | `true` | no |
...
| security\_posture\_config | Mode: Sets the mode of the Kubernetes security posture API's off-cluster features. Available options include DISABLED and BASIC. Vulnerability\_mode:Sets the mode of the Kubernetes security posture API's workload vulnerability scanning. Available options include VULNERABILITY\_DISABLED and VULNERABILITY\_BASIC | `list(object({ mode = string, vulnerability_mode = string }))` | `[]` | no |
